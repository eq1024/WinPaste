...
pub fn init(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    crate::logger::init(app_dir.join("winpaste.log"));
    info!(">>> [STARTUP] winpaste starting up...");
    ...
    let conn = database::init_db(&db_path_str).map_err(|e| {
        ...
        WindowExt::show_error_box("winpaste 启动错误", &err_msg);
        e
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let _ = INPUT_SENDER.set(tx);
        crate::app::hooks::start_input_worker(app_handle.clone(), rx);
        ...
        if let Ok(entries) = std::fs::read_dir(&temp_dir) {
            for entry in entries.flatten() {
                if let Ok(name) = entry.file_name().into_string() {
                    if name.starts_with("winpaste_Clip_") {
...
fn init_announcement_ping(app: &App, repo: &impl SettingsRepository) {
    ...
    if repo.get("app.last_ping_date").unwrap_or(None).as_deref() != Some(&today) {
        ...
        if let Ok(base_url) = std::env::var("WINPASTE_ANNOUNCEMENT_PING_URL") {
...
fn apply_initial_theme(app: &App) {
    ...
    if let Some(window) = app.get_webview_window("main") {
        let _ = crate::app::commands::set_theme(window, app.state::<SettingsState>(), db_state, theme, mode, None, None);
    }
}
