...
pub fn seed_defaults(conn: &Connection) -> Result<()> {
    // App settings
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('app.theme', 'fluent')",
        [],
    );
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('app.color_mode', 'dark')",
        [],
    );
    let _ = conn.execute(
    ...
    let _ = conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('cloud_sync_webdav_base_path', 'winpaste-sync')",
        [],
    );
    let _ = conn.execute(
